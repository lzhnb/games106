...
/*
     ...
     This class is simplified compared to glTF's feature set but retains the basic glTF structure required for this
   sample
...
/*
    ...
    The following functions take a glTF input model loaded via tinyglTF and converts all required data into our own
   structures
*/
...
void VulkanglTFModel::loadImages(tinygltf::Model& input)
{
    ...
    for (size_t i = 0; i < input.images.size(); i++)
    {
        ...
        images[i].texture.fromBuffer(
            buffer, bufferSize, VK_FORMAT_R8G8B8A8_UNORM, glTFImage.width, glTFImage.height, vulkanDevice, copyQueue);
        if (deleteBuffer)
        {
            delete[] buffer;
        }
...
void VulkanglTFModel::loadNode(const tinygltf::Node&                 inputNode,
...
void VulkanglTFModel::drawNode(VkCommandBuffer       commandBuffer,
...
void VulkanExample::setupDescriptors()
{
    ...
    */
    ...
    VK_CHECK_RESULT(
        vkCreateDescriptorSetLayout(device, &descriptorSetLayoutCI, nullptr, &descriptorSetLayouts.jointMatrices));
    ...
    std::array<VkDescriptorSetLayout, 3> setLayouts = {
        descriptorSetLayouts.matrices, descriptorSetLayouts.jointMatrices, descriptorSetLayouts.textures};
    ...
    for (auto& skin : glTFModel.skins)
    {
        const VkDescriptorSetAllocateInfo allocInfo =
            vks::initializers::descriptorSetAllocateInfo(descriptorPool, &descriptorSetLayouts.jointMatrices, 1);
        VK_CHECK_RESULT(vkAllocateDescriptorSets(device, &allocInfo, &skin.descriptorSet));
        VkWriteDescriptorSet writeDescriptorSet = vks::initializers::writeDescriptorSet(
            skin.descriptorSet, VK_DESCRIPTOR_TYPE_STORAGE_BUFFER, 0, &skin.ssbo.descriptor);
        vkUpdateDescriptorSets(device, 1, &writeDescriptorSet, 0, nullptr);
    }
...
void VulkanExample::loadAssets() { loadglTFFile(getAssetPath() + "models/CesiumMan/glTF/CesiumMan.gltf"); }
...
void VulkanExample::viewChanged() { updateUniformBuffers(); }
